...
(fiveam:test test-stand
  ...
  (let ((player-1 (make-player "player-1" '(8 9) 17))
        ...
        (dealer-high-card 10)
        (dealer-low-card 5))
    (fiveam:is-true  (stand player-1 dealer-high-card))
    (fiveam:is-true  (stand player-1 dealer-low-card))
    (fiveam:is-false (stand player-2 dealer-high-card))
    (fiveam:is-true  (stand player-2 dealer-low-card)))
  ;; Second condition
  (let ((player-1 (make-player "player-1" '(6 6) 12))
        (player-2 (make-player "player-2" '(5 6) 11))
        (dealer-high-card 10)
        (dealer-low-card 5))
    (fiveam:is-false (stand player-1 dealer-high-card))
    (fiveam:is-true  (stand player-1 dealer-low-card))
    (fiveam:is-false (stand player-2 dealer-high-card))
    (fiveam:is-false (stand player-2 dealer-low-card))))
